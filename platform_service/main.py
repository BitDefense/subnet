...
async def lifespan(app: FastAPI):
    ...
    try:
        await asyncio.wait_for(
            asyncio.gather(mempool_task, dispatch_task, metagraph_task, return_exceptions=True),
            timeout=5.0
        )
    except asyncio.TimeoutError:
        bt.logging.warning("Shutdown timed out, some tasks may have been killed forcefully.")
    except Exception as e:
